 ...
 * Fixed to address scrolling and cell selection issues
 */
function CanvasGrid({ setContextMenu }) {
  ...
  // Calculate total grid dimensions with proper boundaries
  const getTotalWidth = useCallback(() => {
    // Calculate exact width that covers all columns
    let width = ROW_HEADER_WIDTH;
    for (let col = 0; col < activeSheet.maxCol; col++) {
      width += getColumnWidth(col);
    }
    return Math.max(width, containerRef.current?.clientWidth || 800);
  }, [activeSheet.maxCol, getColumnWidth]);

  const getTotalHeight = useCallback(() => {
    // Calculate exact height needed for visible rows only
    // This prevents extra space below the spreadsheet
    const visibleRows = Math.min(activeSheet.maxRow, 100); // Limit to reasonable number of rows
  ...
  // Fixed with corrected position calculation to address the 6-cell offset
  const getCellIdAtPosition = useCallback((clientX, clientY) => {
    ...
    // Convert client coordinates to grid coordinates without any offset issues
    ...
    // Use direct calculation for row
    // This avoids accumulated rounding errors or rendering discrepancies
    const offsetY = y - COLUMN_HEADER_HEIGHT;
    const rowHeight = getRowHeight(0); // Assuming uniform row height

    // Calculate row index directly from the offset
    const row = Math.floor(offsetY / rowHeight);

    if (row < 0 || row >= activeSheet.maxRow) return null;
    ...
    <div className="canvas-grid-container" data-mode={isSelecting ? "selecting" : "default"}>
      <div
        ...
        style={{
          ...
          scrollbarColor: 'var(--border-color) var(--header-bg)' /* Firefox */
        }}
      >
        <div
          style={{
